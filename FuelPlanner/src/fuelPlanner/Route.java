...
public class Route {
	...
	private void wpBuilder(File routeXML) {
		...
		for (int i = 0; i < wpNodes.size(); i++) { // write lat+lon to this.waypoints
			Namespace nSpace = Namespace.getNamespace(
					"http://www8.garmin.com/xmlschemas/FlightPlan/v1");
			double lat = Double.parseDouble(wpNodes.get(i).
					                        getChild("lat", nSpace).getText());
			double lon = Double.parseDouble(wpNodes.get(i).
					                        getChild("lon", nSpace).getText());
	...
	// use Haversine formula to find distance to next wp in nmi. Gives
	...
	private void distCrsFinder() {
		...
		for (int i = 0; i < waypoints.length - 1; i++) {
			double[] coords = {waypoints[i].getLat() * Math.PI / 180, // lat lon in radians
					           waypoints[i].getLon() * Math.PI / 180, 
					           waypoints[i + 1].getLat() * Math.PI / 180,
					           waypoints[i + 1].getLon() * Math.PI / 180};
			double hav = Math.pow(Math.sin((coords[2] - coords[0]) / 2), 2) +
					Math.cos(coords[0]) * Math.cos(coords[2]) * 
					    Math.pow(Math.sin((coords[3] - coords[1])) / 2, 2);
			...
			double x = Math.cos(coords[0]) * Math.sin(coords[2]) -
					Math.sin(coords[0]) * 
						Math.cos(coords[2]) * Math.cos(coords[3] - coords[1]);
	...
	// Loop through waypoints using user altitude input, setting altitudes
	// which are achievable and calculating fuel burns+time for each leg
	...
	private void wpStats(int i, Airplane airplane) throws IOException  {
		if (waypoints[i].getAlt() == wpAltData[i+1][0]) {
			...
			waypoints[i].setCruiseStats(airplane);
		}
			...
			if (climbDist <= waypoints[i].dist) {
				...
				if (wpAltData[i+1][1] == 0 || wpAltData[i+1][1] == 2) { // if  wp i+1 has unrestricted alt, change its alt
					int newAlt = waypoints[i]
							.climbAltAfter(waypoints[i].dist, airplane);
					waypoints[i+1].setAlt(newAlt);
					wpAltData[i+1][0] = newAlt;
				}
					...
					wpStats(i-1, airplane); //i > 0 since wpAltData[0][1] = 3
				}
